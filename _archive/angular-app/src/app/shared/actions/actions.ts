...
export class Actions {
    public static get(): Action[] {
        var all: Action[] = [
            ...
            new GetBalance(),
            new SimpleAction("From Wei", Module.Utils, "Converts a input value from wei to native value", async (input:ActionInput) => { return Web3.utils.fromWei(input.input, 'ether'); }),
            new SimpleAction("To Wei", Module.Utils, "Converts a decimal value to wei", async (input:ActionInput) => { return Web3.utils.toWei(input.input, 'ether'); }),
            new IsValidSeedPhrase(),
            new SimpleAction("Is Valid Address", Module.Utils, "Checks if an address is a valid address", async (input:ActionInput) => { return String(Web3.utils.isAddress(input.input)); }),
